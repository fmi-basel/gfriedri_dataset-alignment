...
class TileMaskingSection(Section):
    ...
    def __init__(
        ...
        self._tile_smearing_masks = None
    ...
    def create_simple_smearing_masks(self, smear_extend: int) -> None:
        ...
        assert smear_extend < self._tile_overlap * 1.2, "smr_extend must be smaller than 120% over tile-overlap."
    ...
    def get_smearing_mask(self, tile_id: int) -> np.ndarray:
        assert self._tile_smearing_masks is not None, "Tile smearing masks have not been initialized."
    ...
    def save(self, path: str, overwrite: bool = False):
        ...
        tile_smearing_masks_output_dir = Path(path) / self.get_name() / "tile_smearing_masks.npz"
        np.savez(tile_smearing_masks_output_dir, **self._tile_smearing_masks_cache)

    @classmethod
    def load_from_yaml(cls, path: str):
        section = super().load_from_yaml(path)
        ...
        if tile_smearing_masks_path.exists():
            ...
            tile_smearing_masks_cache_path = Path(path).parent / "tile_smearing_masks.npz"
            ...
            section._tile_smearing_masks = None
            section._tile_smearing_masks_cache = {}
